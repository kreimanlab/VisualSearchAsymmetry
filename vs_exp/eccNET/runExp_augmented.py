...
# In[5]:
vgg_model_path_list = [f"../../pretrained_model/vgg16_{x}_filters.h5" for x in ['fisheye', 'lines', 'places365']]
vgg_model_path_list.append(f"../../pretrained_model/vgg16_places365_filters_90.h5")

model_subname_list = ['_fisheye', '_lines', '_places365', '_places365_90']

ecc_models = []

for i in range(len(vgg_model_path_list)):
    vgg_model_path = copy.deepcopy(vgg_model_path_list[i])
    for out_layer in [[1, 1, 1]]:
        model_desc = {'eccParam': eccParam,
                      'ecc_depth': 5,
                      'out_layer': out_layer,
                      'comp_layer': 'diff',
                      'vgg_model_path': copy.deepcopy(vgg_model_path),
                      'model_subname': copy.deepcopy(model_subname_list[i])}
...
for model_desc in ecc_models:
    ...
    if vsm.model_name == "IVSN_1_1_1":
        vsm.model_name = "eccNET_noecc"

    print(vsm.model_name)

    save_path = "out_aug/" + vsm.model_name
    try:
    ...
    for exp_type in exps:
        ...
        for task_id in range(num_task):
            ...
            for i in tqdm(range(NumStimuli), desc=vsm.model_name + " - " + exp_name[task_id]):
